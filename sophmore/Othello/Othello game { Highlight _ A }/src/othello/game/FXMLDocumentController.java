...
public class FXMLDocumentController implements Initializable {
    ...
    List<ImageView> imageHighlights = new ArrayList<>();
...
//                System.out.println("Opposite chip to the right");
    ...
    public boolean checkWinner() {
        //clears all highlighted images
        for (ImageView i : imageHighlights) {
            i.setImage(null);
        }
        ...
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                if((checkMove(x, y, true)) == true) {
                    ...
                    ImageView image = new ImageView("chips/highlight.png");
                    image.setOnMouseClicked(eventOnClick);
                    image.setPickOnBounds(true);

                    imageHighlights.add(image);  // --> adds the image to a list allowing it to be cleared later
        ...
        if (isWinner == true) {
            int blackChipTotal = 0;
            int whiteChipTotal = 0;
            ...
            if (blackChipTotal > whiteChipTotal) {
                JOptionPane.showConfirmDialog(null, "Black: " + blackChipTotal + " White: " + whiteChipTotal, "Black wins!", JOptionPane.PLAIN_MESSAGE);
            }
            else if (whiteChipTotal > blackChipTotal) {
                JOptionPane.showConfirmDialog(null, "White: " + whiteChipTotal + " Black: " + blackChipTotal, "White wins!", JOptionPane.PLAIN_MESSAGE);
            }
            else {
                JOptionPane.showConfirmDialog(null, "Black: " + blackChipTotal + " White: " + whiteChipTotal, "Tie Game!", JOptionPane.PLAIN_MESSAGE);
            }
            
            JOptionPane.showConfirmDialog(null, "Click okay to exit the game!", "Farewell", JOptionPane.PLAIN_MESSAGE);
            System.exit(0);
        }
        return false;
    }
...
//        chips[2][2] = "b";
        ...
        update();
        checkWinner(); // <-- Used for highlighting at the start of the game (do not remove)
